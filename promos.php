    ...
    <main class="shop-page">
        <div class="container">
            <div class="shop-header">
                ...
                <p>Limited-time offers and exclusive deals (Items under ₱5,000)</p>
            </div>
            <div class="products-grid" id="productsGrid">
                <p style="text-align:center; width:100%;">Loading promos...</p>
            </div>
        </div>
    </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            fetch('api/products.php')
                ...
                .then(data => {
                    // Logic: Filter items cheaper than 5000 to simulate "Promos"
                    const promoItems = data.filter(p => parseFloat(p.price) < 5000);
                    
                    if (promoItems.length > 0) {
                        displayProducts(promoItems);
                    } else {
                        document.getElementById('productsGrid').innerHTML = '<p>No current promotions available.</p>';
                    }
                })
                .catch(err => {
                    console.error('Error:', err);
                    document.getElementById('productsGrid').innerHTML = '<p>Error loading promos.</p>';
                });
        });
    </script>
</body>
