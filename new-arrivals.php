...
<head>
    ...
    <title>New Arrivals - YanKicks</title>
    ...
    <main class="shop-page">
        <div class="container">
            ...
            <div class="products-grid" id="productsGrid">
                <p style="text-align:center; width:100%;">Loading new arrivals...</p>
            </div>
        </div>
    </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            fetch('api/products.php')
                ...
                .then(data => {
                    // API sends newest first (id DESC). 
                    // So we take the first 3 items (slice 0 to 3)
                    const newArrivals = data.slice(0, 3);
                    
                    if (newArrivals.length > 0) {
                        displayProducts(newArrivals);
                    } else {
                        document.getElementById('productsGrid').innerHTML = '<p>No new arrivals found.</p>';
                    }
                })
                .catch(err => {
