...
<head>
    ...
    <title>Featured Products - YanKicks</title>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            fetch('api/products.php')
                ...
                .then(data => {
                    // For "Featured", maybe limit to first 8 items or specific ones
                    // This example just shows all, but you can filter if needed
                    const featuredProducts = data.slice(0, 8); 
                    
                    if (featuredProducts.length > 0) {
                        displayProducts(featuredProducts);
                    } else {
                        document.getElementById('productsGrid').innerHTML = '<p>No featured products found.</p>';
                    }
                })
                .catch(err => {
